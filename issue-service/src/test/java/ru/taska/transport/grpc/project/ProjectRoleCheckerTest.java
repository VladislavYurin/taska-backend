...
class ProjectRoleCheckerTest {
    ...
    void checkProjectRole_shouldCompleteSuccessfully_whenAccessIsAllowed() {
        ...
        Mockito.when(client.checkProjectAccess(
        ...
        StepVerifier.create(projectAccessChecker.checkProjectAccess(
                        REQUEST_ID, NODE_ID, PROJECT_ID, USER_ID, ALLOWED_ROLES)
                )
                .verifyComplete();

        Mockito.verify(client).checkProjectAccess(
                REQUEST_ID, NODE_ID, PROJECT_ID, USER_ID
        );
    }
    ...
    void checkProjectRole_shouldThrowException_whenUserNotProjectMember() {
        var result = CheckProjectAccessResponse.newBuilder()
                .setIsMember(false)
                .setProjectExists(true)
                .setProjectArchived(false)
                .build();
