    ...
    public Mono<LoginResponse> login(Mono<LoginRequest> request) {
        return request
                ...
                .flatMap(t -> {
                    ...
                    log.info("[{}][{}] Login request for email: {}", requestId, nodeId, email);

                    return authService.login(email, password)
                            .doOnSuccess(response -> log.info("[{}][{}] Login successful for email: {}",
                                    requestId, nodeId, email))
                            .doOnError(error -> log.error("[{}][{}] Login failed for email: {}",
